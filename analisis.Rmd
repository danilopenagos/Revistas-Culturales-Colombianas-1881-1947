...
#           colnames = c("ID", "Nombre de la revista", "Número de contribución", "Autor", "Rol", "Seudónimo/Firma", "País de Origen", "Título", "Fecha publicación", "Fecha de publicación ISO", "Tipo de texto", "Traducción (Sí/No)", "Traductor", "Cantidad de páginas", "Paginación"),
...
# 
# colaboradores_genero_pais <- select(revistas_datos_bio, Colaborador, Tipo, PaisOrigen.x)
# colaboradores_genero_pais <- colaboradores_genero_pais |>
#    group_by(Tipo, PaisOrigen.x) |>
#    dplyr::filter(Colaborador != "Anónimo") |>
#    count(Colaborador)
# 
# total_colaboraciones_genero <- colaboradores_genero_pais |>
#    group_by(Colaborador) |>
#    summarise(TotalTextos = sum(n))
# #
# top_100_colaboradores <- total_colaboraciones_genero|>
#    top_n(100, wt = TotalTextos) |>
#    pull(Colaborador)
# 
#  top_colaboradores_revistas_genero <- colaboradores_genero_pais |>
#    group_by(Colaborador) |>
#    mutate(Total_textos = sum(n)) |>
#    ungroup() |>
#    arrange(desc(Total_textos), Colaborador) |>
#    dplyr::filter(Colaborador %in% top_100_colaboradores)

# write.csv(top_colaboradores_revistas_genero, file = "top_100_colaboradores_genero.csv", fileEncoding = "UTF-8", row.names = F)
